...
Dlambda = ((data.So/4) * (data.lambda2 - (data.lambda0**4/data.lambda2**3)))
...
DresMax = (fatorMeritoMax * (2 * math.pi) * data.c) / ((data.ritmoBinario ** 2) * (data.lambda2 ** 2))  # s/m
...
def interpolation_values():
    ...
    for i in data.val_data_sheet:
        ...
        x = round(((data.lambda2*10**9)-b)/declive,2)
        results1.append(x)
...
def dcf_obj():
    ddcf_obj = [round(((DresMax*1000)/(len(data.lengths_section)-1))-(Dlambda_psnmkm*l),2) for l in data.lengths_section]
    # print(DresMax*1000)
...
# disperção ssmf
def disp_ssmf():
    disp_ssmf = [round(Dlambda_psnmkm * l,2) for l in data.lengths_section]
